...
export type CreateTableData = { tenantId: string; namespaceId: string };
...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>Create lookup table</h2>
    <mat-dialog-content>
...
export class CreateTableDialogComponent {
  ...
  private readonly ref = inject(MatDialogRef<CreateTableDialogComponent, LookupTable | undefined>);
  ...
  constructor(@Inject(MAT_DIALOG_DATA) readonly data: CreateTableData) {}

  save(): void {
    this.saving = true;
    this.api
      .createTable(this.data.tenantId, this.data.namespaceId, {
        name: this.name.trim(),
        slug: this.slug.trim(),
      })
      .subscribe({
